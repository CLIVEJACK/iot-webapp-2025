...
### VS Tip
- 이전 솔루션을 복사하고 나서 MSBuild에서 오류가 발생한 가능성이 있음
...
1. 회원가입 / 로그임 계속
    - 자동색성 테이블
        - AspNetRoleClaims : 역할 내 모든 사용자에게 부여되는 크레임(권리 - 편집가능, 작성가능,...)
        - AspNetRoles : 역할저장 테이블(관리자, 사용자, 뷰어...)
        - AspNetUesrClaims : 사용자가 소유한 클레임
        - AspNetUserLogins : 사용자테이블, 역할테일을 연결하는 Join entity
        - `AspNetUsers` : 사용자정보 테이블
        - AspNetUserTokens : 사용자 인증토큰 테이블 
    - Models.RegisterModel 클래스터 생성
2. 게시판 준비
3. 이후 작업
