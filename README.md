...
### 웹 표현기술 - CSS
- HTML,CSS, JS 동일하게 "(쌍따옴표)", '(홀따옴표)' 동시 사용가능
- Pythondms ''를 추천, 웹은 ""를 추천
- 스타일부터 레이아수 까지 연습
- [소스](./day02/html04.html)

### 웹 표준기술 - JavaScript
- Java(컴파일러언어)와 아무런관련없음 ex) 햄이랑 햄스터랑 같은건 아니다, 인도와 인도네시아 같은 느낌
- JavaScript(스크립트 언어)
...
- DOM에서 쓰는 주요 함수들
    ...
    ```js
        ...
        // 여러개 객체 선택
        document.getSelectorAll(선택자) //for문으로 후처리
        document.getElementsByName(이름)
        document.getElementsByClassName(클래스)

        // 객체변수
        var obj = document.getElenmentById("container");
...
- CDN 방식으로 사용
    ...
    ```
- 모든 DOM객체에 접근할때 '$(선택자|아이디|클래스)'로 처리
...
#### Vanilla JS
- 순수 JS로 불리며 jQuery처럼 con링크가 필요없음
- 전통적인 방식보다 효율적으로 변경됨
...
- HTML5 + CSS3 + JS + BootStrap 응용예제
    ...
    https://github.com/user-attachments/assets/8780a750-08af-49db-a2e5-2bb9fd64744f
...
https://github.com/user-attachments/assets/542a3abb-6988-4db5-a881-8812ba0234d3
...
- PATCH - 리소스 부분 변경(수정). 거의 사용안함. PATCH 메서드를 지원하지 않으면 PUT으로 대체
...
2. Python에서 100만건 더미데이터 생성
3. WebAPI 프로젝트 생성
4. EntityFramework NuGet패키지 설치
5. appsettings.json DB연결문자열 추가
6. MySQL iot_datas 테이블 생성
7. Models.AppDbContext 클래스 생성
8. Models.cs AppDbContext 클래스 생성
...
- ASP.NET Core 프로젝트에서 정적페이지HTML, 동적페이지 cshtml 동시 사용가능
- 웹사이트만 새로 생성
    1. ASP.NET Core 비어있음 선택
    2. wwwroot/html/index.html 파일 생성 - VS Code의 Live Sever예제와 동일(정적페이지)
    3. Program.cs에서 MVC패턴 관련된 로직 추가
    4. Controllers/HomeControoler.cs 생성
    5. Index()메서드에서 뷰 추가 - ASP.NET Core 동적페이지
    6. Views() 폴더 하위 
        - _ViewImport.cshtml
        - _ViewStart.cshtml
        - Shated/_Layout.cshtml
    7. index.html에 화면UI 구현 

- AJAX : Asyncronous Javascript And xml. 자바스크립트에서 비동기로 메서드를 호출 기술
    - 예전에 XML로만 데이터 전달. 현재는 Json으로 이전중

- CORS Policy Block : Cross-Origin Resource sharing. 다른 출처 리소스 접근허용 보안 메커니즘 
    - 아무나 URL로 호출을 못하도록 웹페이지 보안설정
    - WebAIP 서비스에서 Program.cs에서 CORS호출권한 설정 추가
    - 프론트엔드는 CORS 설정 필요없음
