...
namespace WpfTodoListApp
{
    ...
    public partial class MainWindow : MetroWindow
    {
        ...
        // ObservableCollections을 굳이 사용하지 않아도 문제는 되지않음
        ...
        private async Task GetDatas()
        {
            ...
            try // API 호출
            {
                // http://localhost:6200/api/TodoItmes
        ...
        // async시 Task가 리턴값이지만 버튼클릭 이벤트 메서드와 연결시는 Task -> void로 변경해줘야 연결 가능
        ...
        private async void GrdTodoItems_SelectedChanged(object sender, System.Windows.RoutedEventArgs e)
        {

            try
            {
                //await this.ShowMessageAsync("클릭", "클릭확인");
                var Id = (GrdTodoItems.SelectedItem as TodoItem)?.Id;   // ?. -> Null이 생겨도 예외발생 안함
                if (Id == null) return; // 이 구분을 만나야 아래 로직이 실행안됨
        ...
        private async void BtnDelete_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            try
            {
                var Id = Convert.ToInt32(TxtId.Text); // 삭제는 Id만
